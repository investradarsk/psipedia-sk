CREATE TABLE `partner_new_profile_metadata` (
  ...
  `intended_resource_type` text NOT NULL CHECK (`intended_resource_type` IN ('DIRECTORY_PROFILE','HELP_ORGANIZATION')),
  `display_name` text NOT NULL,
  `category_or_type` text NOT NULL,
  ...
  `resolution_type` text CHECK (`resolution_type` IS NULL OR `resolution_type` IN ('CREATED_NEW','LINKED_EXISTING')),
  `resolved_resource_id` text REFERENCES `partner_resources`(`id`) ON DELETE RESTRICT,
  `resolved_canonical_id` integer,
  `dedupe_active` integer NOT NULL DEFAULT 1 CHECK (`dedupe_active` IN (0,1)),
  `created_at` text NOT NULL
);
CREATE UNIQUE INDEX `partner_new_profile_active_identity_unique`
  ON `partner_new_profile_metadata` (`partner_account_id`,`identity_fingerprint`)
  WHERE `dedupe_active`=1;
CREATE INDEX `partner_new_profile_account_created_idx`
  ON `partner_new_profile_metadata` (`partner_account_id`,`created_at`);
CREATE INDEX `partner_new_profile_duplicate_created_idx`
  ON `partner_new_profile_metadata` (`duplicate_confidence`,`created_at`);
CREATE INDEX `partner_new_profile_resolution_idx`
  ON `partner_new_profile_metadata` (`resolution_type`,`resolved_canonical_id`);
