CREATE TABLE `partner_event_submission_metadata` (
  ...
  `duplicate_confidence` text NOT NULL DEFAULT 'NONE' CHECK (`duplicate_confidence` IN ('NONE','MEDIUM','HIGH')),
  `duplicate_candidate_id` integer REFERENCES `managed_events`(`id`) ON DELETE RESTRICT,
  `duplicate_reasons_json` text NOT NULL DEFAULT '[]',
  `resolution_type` text CHECK (`resolution_type` IS NULL OR `resolution_type` IN ('CREATED_NEW','LINKED_EXISTING','UPDATED')),
  `resolved_event_id` integer REFERENCES `managed_events`(`id`) ON DELETE RESTRICT,
  `created_at` text NOT NULL
);
CREATE UNIQUE INDEX `partner_event_submission_active_dedupe_unique`
  ON `partner_event_submission_metadata` (`partner_account_id`,`dedupe_key`)
  WHERE `dedupe_active`=1;
CREATE INDEX `partner_event_submission_resource_created_idx`
  ON `partner_event_submission_metadata` (`partner_resource_id`,`created_at`);
CREATE INDEX `partner_event_submission_account_created_idx`
  ON `partner_event_submission_metadata` (`partner_account_id`,`created_at`);
CREATE INDEX `partner_event_submission_duplicate_created_idx`
  ON `partner_event_submission_metadata` (`duplicate_confidence`,`duplicate_candidate_id`,`created_at`);
CREATE INDEX `partner_event_submission_resolution_idx`
  ON `partner_event_submission_metadata` (`resolution_type`,`resolved_event_id`);
