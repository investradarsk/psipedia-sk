...
function fixture() {
  ...
  for (let i = 1; i <= 151; i++) insert.run(i, `pes-${i}`, i === 151 ? "Labrador % Rex" : `Pes ${i}`, i <= 36 ? "adopcia" : "dobrovolnictvo", i <= 36 ? "draft" : "published", i === 151 ? "OZ Rex" : "Pomoc", i === 151 ? "Rex" : "", i === 151 ? "Nitra" : "Trnava", i === 37 ? 1 : 0, String(i).padStart(3, "0"));
  insert.run(152, "legacy-utulok", "Legacy útulok", "utulky", "published", "Legacy OZ", "", "Trnava", 0, "152");
  // Legacy shelter row is intentionally retained physically but hidden from this admin surface.
...
test("counts cover all 151 records while list is paged at 50", async () => {
  ...
  assert.equal(result.categoryCounts.dobrovolnictvo, 115);
  assert.equal(result.categoryCounts.utulky, undefined);
...
test("category and status independently and together filter full database", async () => {
  ...
  assert.equal((await queryHelpAdmin(db, filters({ category: "utulky", status: "published" }))).resultCount, 0);
...
test("invalid URL values fall back safely and filters retain valid values", () => {
  ...
  assert.deepEqual(parseHelpAdminFilters(new URLSearchParams("category=utulky")), filters());
});
