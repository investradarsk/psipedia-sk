...
test.describe("MAP-1B admin geo foundation", () => {
  ...
  test("geo operations shows gates and never exposes an unbounded full-backfill action", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "Inicializovať max. 20 riadkov" })).toBeVisible();
    await expect(page.getByRole("button", { name: "Spustiť max. 5 canary requestov" })).toBeVisible();
