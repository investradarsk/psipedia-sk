...
const SEEDED_REVIEW_HREF = "/recenzie/e2e-testovacia-recenzia-krmiva";

async function robotsContent(page: import("@playwright/test").Page) {
  const robots = page.locator('meta[name="robots"]');
  await expect(robots).toHaveCount(1);
  return (await robots.getAttribute("content")) ?? "";
}
...
test("Phase 5 robots metadata is singular and follows indexability rules", async ({ page }) => {
  await page.goto("/recenzie");
  const hubRobots = await robotsContent(page);
  expect(hubRobots).toMatch(/\bindex\b/i);
  expect(hubRobots).not.toMatch(/\bnoindex\b/i);

  await page.goto("/recenzie/maskrty");
  const thinCategoryRobots = await robotsContent(page);
  expect(thinCategoryRobots).toMatch(/\bnoindex\b/i);
  expect(thinCategoryRobots).toMatch(/\bfollow\b/i);

  for (const href of ["/recenzie/krmiva", SEEDED_REVIEW_HREF]) {
    await page.goto(href);
