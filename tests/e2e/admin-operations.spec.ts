...
test("operations queue is authenticated, readable, responsive and mutation-free", async ({ page }) => {
  ...
  await expect(page.getByRole("form", { name: "Filtrovať attention queue" })).toHaveAttribute("method", "get");
  await expect(page.getByRole("link", { name: "Operácie", exact: true })).toBeVisible();

  const mutationLabels = /schváliť|zamietnuť|publikovať|archivovať|vybavené|vymazať|odoslať|importovať/i;
  await expect(page.getByRole("button", { name: mutationLabels })).toHaveCount(0);
