  ...
  return value.split(pattern).filter(Boolean).map((part, index) => {
    ...
    if (link) {
      ...
      return href.startsWith("/")
        ...
        : <a key={index} href={href} target="_blank" rel="noreferrer">{link[1]}<span className="sr-only"> (otvorí sa v novom okne)</span></a>;
    }
...
export function ArticleBlocks({ blocks, preview = false }: { blocks: ArticleBlock[]; preview?: boolean }) {
  return (
    <div className={preview ? "article-blocks article-blocks--preview" : "article-blocks"}>
      {blocks.map((block) => {
        if (block.type === "text") return block.content ? <p key={block.id}>{richText(block.content)}</p> : null;
        ...
        if (block.type === "image") return block.url ? (
          <figure className="article-block-image" key={block.id}>
        ...
        if (block.type === "tip") return block.content ? <aside className="article-block-callout article-block-callout--tip" key={block.id}><strong>Tip z praxe</strong><p>{richText(block.content)}</p></aside> : null;
        if (block.type === "warning") return block.content ? <aside className="article-block-callout article-block-callout--warning" key={block.id}><strong>Dôležité upozornenie</strong><p>{richText(block.content)}</p></aside> : null;
        if (block.type === "quote") return block.content ? <blockquote className="article-block-quote" key={block.id}><p>{richText(block.content)}</p>{block.attribution && <cite>{block.attribution}</cite>}</blockquote> : null;
        ...
        if (block.type === "source") {
          ...
          return block.label && href ? <aside className="article-block-source" key={block.id}><strong>Odborné zdroje</strong><a href={href} target="_blank" rel="noreferrer">{block.label} ↗</a>{block.note && <p>{richText(block.note)}</p>}</aside> : null;
        }
