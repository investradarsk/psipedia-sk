  ...
  return <>
    ...
    <section className="admin-panel"><div className="admin-toolbar"><div className="admin-status-filter" aria-label="Filtrovať návrhy podľa stavu">{([['all','Všetky'],['new','Nové'],['approved','Schválené'],['rejected','Zamietnuté']] as const).map(([value,label]) => <button type="button" className={status === value ? "is-active" : ""} onClick={() => setStatus(value)} key={value}>{label}</button>)}</div></div>
      ...
      {visible.length ? <div className="admin-change-list">{visible.map((item) => {
        ...
        return <article id={`navrh-${item.id}`} className={`admin-change-card is-${item.status}`} key={item.id}><header><div><span className={`admin-inquiry-status is-${item.status}`}>{statusLabels[item.status]}</span><span>{formatDate(item.createdAt)}</span></div><h2>{item.profileName}</h2><p>{directoryCategoryLabel(item.profileCategory)} · <Link href={`/adresar/${item.profileCategory}/${item.profileSlug}`} target="_blank">Verejný profil ↗</Link></p></header>
