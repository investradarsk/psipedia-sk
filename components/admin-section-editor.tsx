    ...
    <div className="admin-section-list">{sections.map((section, index) => <article className="admin-section-item" key={section.slug}>
      ...
      {open === section.slug && <div className="admin-section-body">
        ...
        <div className="admin-subsection-list">{section.subpages.map((subpage, subIndex) => {
          ...
          const isStructuredSection = section.slug === "starostlivost" || section.slug === "aktivity" || section.slug === "steniatka" || isReviewSection;
          ...
          return <article className="admin-subsection-row" key={`${subpage.slug}-${subIndex}`}>
            ...
            {isStructuredSection && <details className={`admin-care-area-fields ${isActivitySection ? "admin-activity-area-fields" : ""} ${isPuppySection ? "admin-puppy-area-fields" : ""}`} open={subIndex === 0}>
              <summary>{isReviewSection ? "Obsah kategórie recenzií a testov" : isActivitySection ? "Obsah oblasti výcviku a aktivít" : isPuppySection ? "Obsah oblasti Šteniatok" : "Obsah poradenskej oblasti"}</summary>
              <p>Každú položku zoznamu napíš na samostatný riadok. Odkazy zapisuj ako <code>Názov | /adresa</code>.</p>
              <div className="admin-field-grid">
                <label>{isReviewSection ? "Témy a kritériá výberu" : isActivitySection ? "Aktivity a témy" : isPuppySection ? "Témy tejto fázy" : "Najčastejšie témy"}<textarea rows={5} value={lines(subpage.popularTopics)} onChange={(event) => updateSubpage(section.slug, subIndex, { popularTopics: fromLines(event.target.value) })} /></label>
                ...
                <label>{isReviewSection ? "Ako vyberať a čo porovnávať" : isActivitySection ? "Ako začať" : isPuppySection ? "Čo urobiť teraz" : "Čo sledovať alebo urobiť doma"}<textarea rows={6} value={lines(subpage.homeSteps)} onChange={(event) => updateSubpage(section.slug, subIndex, { homeSteps: fromLines(event.target.value) })} /></label>
                <label>{isReviewSection ? "Na čo si dať pozor" : isActivitySection ? "Bezpečnosť a limity" : isPuppySection ? "Na čo si dať pozor" : "Varovné signály"}<textarea rows={6} value={lines(subpage.warningSigns)} onChange={(event) => updateSubpage(section.slug, subIndex, { warningSigns: fromLines(event.target.value) })} /></label>
                <label className="admin-field--full">{isReviewSection ? "Metodika testovania a redakčný kontext" : isActivitySection ? "Čo zvážiť pri výbere" : isPuppySection ? "Dôležité pre túto fázu" : "Kedy vyhľadať odborníka"}<textarea rows={3} value={subpage.expertAdvice ?? ""} onChange={(event) => updateSubpage(section.slug, subIndex, { expertAdvice: event.target.value })} /></label>
                <label>{isReviewSection ? "Súvisiace odkazy a služby" : isActivitySection || isPuppySection ? "Kontakty a súvisiace služby" : "Užitočné kontakty"}<textarea rows={4} value={serviceLines(subpage.serviceLinks)} onChange={(event) => updateSubpage(section.slug, subIndex, { serviceLinks: fromServiceLines(event.target.value) })} /></label>
