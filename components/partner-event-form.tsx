...
export function PartnerEventForm({mode,resourceId,baseRevision,initial}:Props){
  ...
  async function submit(event:React.FormEvent){
    ...
    try{
      if(mode==="create"){
        const response=await fetch("/api/partner/events",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({event:draft,confirmDuplicate})});
        ...
        if(!Object.keys(patch).length)throw new Error("Nezmenili ste žiadny údaj.");
        const response=await fetch(`/api/partner/events/${encodeURIComponent(resourceId)}/changes`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({baseRevision,patch})});
