      ...
      <div className="admin-block-list">
        ...
        {blocks.map((block, index) => (
          ...
          >
            ...
            {block.type === "source" && <div className="admin-field-grid"><div className="admin-field"><label>Názov zdroja</label><input value={block.label} onChange={(event) => update(block.id, (item) => item.type === "source" ? { ...item, label: event.target.value } : item)} placeholder="Napríklad AVMA" /></div><div className="admin-field"><label>Odkaz na zdroj</label><input type="url" value={block.url} onChange={(event) => update(block.id, (item) => item.type === "source" ? { ...item, url: event.target.value } : item)} placeholder="https://…" /></div><div className="admin-field admin-field--full"><label>Poznámka (nepovinná)</label><input value={block.note ?? ""} onChange={(event) => update(block.id, (item) => item.type === "source" ? { ...item, note: event.target.value } : item)} /></div></div>}
