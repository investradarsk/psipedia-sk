...
async function AdminNavigation() {
  ...
  return (
    <nav className="admin-section-nav" aria-label="Redakčné moduly">
      ...
      <div className="admin-nav-group"><span>Komunita</span><div><Link href="/admin/tipy">Tipy</Link><Link href="/admin/hodnotenia">Hodnotenia</Link><Link href="/admin/dopyty">Dopyty{newInquiryCount > 0 ? ` ${newInquiryCount}` : ""}</Link></div></div>
