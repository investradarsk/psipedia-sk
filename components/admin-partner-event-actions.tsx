...
export function AdminPartnerEventActions({id,operation,candidateId}:{id:string;operation:"CREATE"|"UPDATE";candidateId?:number|null}){
 const router=useRouter();const [reasonCode,setReasonCode]=useState("INCORRECT_INFORMATION");const [canonicalId,setCanonicalId]=useState(candidateId?String(candidateId):"");const [busy,setBusy]=useState(false);const [message,setMessage]=useState("");
 ...
 return <section className="admin-form-card"><h2>Rozhodnutie</h2><div className="admin-commercial-actions">
  {operation==="CREATE"?<><button type="button" disabled={busy} onClick={()=>run("CREATE_EVENT")}>CREATE EVENT (DRAFT)</button><label>Existujúce event ID<input type="number" min="1" value={canonicalId} onChange={e=>setCanonicalId(e.target.value)} disabled={busy}/></label><button type="button" disabled={busy||!canonicalId} onClick={()=>run("LINK_EXISTING")}>LINK EXISTING</button></>:<button type="button" disabled={busy} onClick={()=>run("APPROVE")}>Schváliť zmeny</button>}
