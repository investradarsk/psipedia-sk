...
import {listPartnerCommercialAgreementsAdmin} from "@/lib/partner-commercial-agreements";
...
export default async function Page({searchParams}:{searchParams:Promise<Search>}){
  ...
  return <AdminShell user={user} eyebrow="Partner commercial" title="Komerčný hub" description="Leady, manuálne dohody, platby a platené aktivácie bez online payment gateway.">
    <section className="admin-form-card"><h2>Prehľad</h2><div className="admin-commercial-list">
      ...
      <article><strong>{awaiting}</strong><span>čaká na platbu</span></article>
      <article><strong>{ready}</strong><span>pripravené na aktiváciu</span></article>
      <article><strong>{active}</strong><span>aktívne komerčné položky</span></article>
      <article><strong>{expiring}</strong><span>končí do 14 dní</span></article>
    </div></section>
