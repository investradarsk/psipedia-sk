...
import { articleHref, portalSections, portalSubpageHref } from "@/lib/portal";
import { SITE_URL } from "@/lib/seo";

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const [articles, events, directoryProfiles, helpCases] = await Promise.all([getPublishedArticles(), getPublishedEvents(), getPublishedDirectoryProfiles(), getPublishedHelpCases()]);
  ...
  return [
    ...
    ...breeds.map((breed) => ({ url: `${SITE_URL}/plemena/${breed.slug}`, lastModified: new Date("2026-08-17"), changeFrequency: "monthly" as const, priority: 0.8, images: [`${SITE_URL}${breed.image}`] })),
