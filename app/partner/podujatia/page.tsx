...
import { partnerRoleLabel } from "@/lib/partner-ui-labels";

export const dynamic="force-dynamic";

function dateLabel(value:string){
  return new Intl.DateTimeFormat("sk-SK",{day:"numeric",month:"long",year:"numeric",timeZone:"Europe/Bratislava"}).format(new Date(value+"T12:00:00Z"));
}

export default async function Page(){
 const identity=await requirePartnerPageIdentity();
 const items=await listPartnerManagedEvents(identity.accountId);
 return <PartnerShell title="Moje podujatia" description="Podujatia priradené k vášmu Partner účtu. Úpravy vždy prechádzajú moderáciou.">
  <div className="partner-new-profile-cta">
    <div><span className="eyebrow">Podujatia</span><h2>Chýba vaše podujatie?</h2><p>Navrhnite nové podujatie. Po schválení vznikne ako koncept a redakcia rozhodne o publikovaní.</p></div>
    <Link href="/partner/podujatia/nove" className="button button--dark">Pridať podujatie</Link>
  </div>
  {items.length?<div className="partner-resource-grid">{items.map(item=><article className="partner-resource-card" key={item.resourceId}>
    ...
    <p>Rola: <strong>{partnerRoleLabel(item.role)}</strong></p>
  ...
  </article>)}</div>:<section className="partner-empty"><h2>Zatiaľ nemáte priradené podujatie</h2><p>Môžete navrhnúť nové podujatie. Po schválení sa zobrazí medzi podujatiami, ktoré môžete spravovať.</p></section>}
 </PartnerShell>;
}
