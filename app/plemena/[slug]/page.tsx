...
export default async function BreedDetailPage({ params }: Props) {
  ...
  const sections = [{id:"povaha",eyebrow:"Povaha",title:"Povaha a temperament",content:breed.character},
    {id:"historia",eyebrow:"História",title:"Pôvod a vývoj plemena",content:breed.history},
    {id:"pohyb",eyebrow:"Pohyb",title:"Pohyb a denné potreby",content:breed.exercise || breed.needs},
    {id:"vycvik",eyebrow:"Výcvik",title:"Výcvik a výchova",content:breed.training},
    {id:"zdravie",eyebrow:"Zdravie",title:"Zdravie a starostlivosť",content:breed.health},
    {id:"srst",eyebrow:"Starostlivosť",title:"Srsť a údržba",content:editorial.coatCare},
    ...
    {id:"zaujimavosti",eyebrow:"Zaujímavosti",title:"Čím je plemeno výnimočné",content:editorial.curiosities},
  ...
  const hasPracticalContent = sections.length > 0 || healthRisks.length > 0 || sources.length > 0;
  ...
  return <main id="obsah">
    ...
    <header className="breed-detail-hero shell" id="prehlad"><nav className="article-breadcrumbs" aria-label="Navigácia"><Link href="/">Domov</Link><span>/</span><Link href="/plemena">Plemená</Link><span>/</span><span>{breed.name}</span></nav>
      <div className={`breed-detail-card${breed.image ? "" : " breed-detail-card--placeholder"}`}><div className="breed-detail-intro">{managedBreed?.fciNumber && <div className="breed-hero-meta"><span>FCI č. {managedBreed?.fciNumber}</span></div>}<h1>{breed.name}</h1>{managedBreed?.officialFciName && <p className="breed-official-name">{managedBreed?.officialFciName}</p>}<div className="breed-fci-path" aria-label="FCI zaradenie plemena"><Link href={fciGroupHref} className="breed-fci-path-node" aria-label={`FCI skupina ${breed.fciGroup}: ${fciGroupName}`}><small>FCI skupina {breed.fciGroup}</small><strong>{fciGroupName}</strong></Link>{(fciSectionNumber || fciSectionName) && <><span className="breed-fci-path-arrow" aria-hidden="true">→</span>{fciSectionHref ? <Link href={fciSectionHref} className="breed-fci-path-node" aria-label={`FCI sekcia ${fciSectionNumber}${fciSectionName ? `: ${fciSectionName}` : ""}`}><small>Sekcia {fciSectionNumber}</small>{fciSectionName && <strong>{fciSectionName}</strong>}</Link> : <span className="breed-fci-path-node"><small>FCI sekcia</small>{fciSectionName && <strong>{fciSectionName}</strong>}</span>}</>}<span className="breed-fci-path-arrow" aria-hidden="true">→</span><span className="breed-fci-path-current"><small>Plemeno</small><strong>{breed.name}</strong></span></div>{breed.intro && <p className="breed-lead">{breed.intro}</p>}<dl className="breed-hero-facts">{breed.origin && <div><dt>Pôvod</dt><dd>{breed.origin}</dd></div>}{heroHeight && <div><dt>Výška</dt><dd>{heroHeight}</dd></div>}{heroWeight && <div><dt>Hmotnosť</dt><dd>{heroWeight}</dd></div>}{breed.lifespan && <div><dt>Dĺžka života</dt><dd>{breed.lifespan}</dd></div>}{fci.vyuzitie && <div><dt>Využitie</dt><dd>{fci.vyuzitie}</dd></div>}</dl></div>{breed.image ? <img className="breed-detail-image" src={breed.image} alt={`${breed.name} – titulná fotografia plemena`} /> : <div className="breed-detail-image breed-detail-image--empty" role="img" aria-label={`Fotografia plemena ${breed.name} sa pripravuje`}><span className="breed-placeholder-mark" aria-hidden="true"><PawMark size={68}/></span><span>Fotografia sa pripravuje</span><small>Profil doplníme o autentický záber plemena.</small></div>}</div></header>
    ...
    <section className="breed-at-a-glance shell" aria-labelledby="breed-at-a-glance-title"><header><span className="eyebrow">Rýchla orientácia</span><h2 id="breed-at-a-glance-title">Plemeno v skratke</h2></header><div className="breed-at-a-glance-grid"><dl>{breed.origin&&<div><dt>Pôvod</dt><dd>{breed.origin}</dd></div>}{heroHeight&&<div><dt>Výška</dt><dd>{heroHeight}</dd></div>}{heroWeight&&<div><dt>Hmotnosť</dt><dd>{heroWeight}</dd></div>}{breed.lifespan&&<div><dt>Dĺžka života</dt><dd>{breed.lifespan}</dd></div>}{fci.vyuzitie&&<div><dt>Využitie</dt><dd>{fci.vyuzitie}</dd></div>}{managedBreed?.workingTrial&&<div><dt>Pracovná skúška</dt><dd>{managedBreed?.workingTrial}</dd></div>}</dl>{managedBreed?.editorialComplete&&<div className="breed-at-a-glance-ratings"><RatingDots value={managedBreed.energy} label="Energia"/><RatingDots value={managedBreed.trainability} label="Cvičiteľnosť"/><RatingDots value={managedBreed.children??managedBreed.family} label="Vzťah k deťom"/>{typeof managedBreed.otherDogs==="number"&&<RatingDots value={managedBreed.otherDogs} label="Vzťah k psom"/>}{typeof managedBreed.apartment==="number"&&<RatingDots value={managedBreed.apartment} label="Vhodnosť do bytu"/>}{typeof managedBreed.grooming==="number"&&<RatingDots value={managedBreed.grooming} label="Starostlivosť"/>}{typeof managedBreed.shedding==="number"&&<RatingDots value={managedBreed.shedding} label="Pĺznutie"/>}{typeof managedBreed.preyDrive==="number"&&<RatingDots value={managedBreed.preyDrive} label="Lovecký inštinkt"/>}</div>}</div></section>
    {(breed.goodFor.length>0||breed.consider.length>0)&&<section className="breed-fit-guide shell" aria-label="Praktická vhodnosť plemena">{breed.goodFor.length>0&&<div><span className="eyebrow">Dobrá zhoda</span><h2>Pre koho sa hodí</h2><ul>{breed.goodFor.map((item)=><li key={item}>{item}</li>)}</ul></div>}{breed.consider.length>0&&<div><span className="eyebrow">Pred rozhodnutím</span><h2>Na čo si dať pozor</h2><ul>{breed.consider.map((item)=><li key={item}>{item}</li>)}</ul></div>}</section>}
    ...
    <section className="breed-useful-links shell"><h2>Užitočné služby a kontakty</h2><div><Link href={`/adresar/treneri?breed=${encodeURIComponent(breed.name)}`}>Psí tréneri</Link><Link href="/adresar/kynologicke-kluby">Kynologické kluby</Link><Link href="/adresar/chovatelske-kluby">Chovateľské kluby</Link><Link href="/adresar/chovatelske-stanice">Chovateľské stanice</Link></div></section>
