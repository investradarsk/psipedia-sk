...
export default async function BreedDetailPage({ params }: Props) {
  ...
  return <main id="obsah">
    ...
    <header className="breed-detail-hero shell" id="prehlad"><nav className="article-breadcrumbs" aria-label="Navigácia"><Link href="/">Domov</Link><span>/</span><Link href="/plemena">Plemená</Link><span>/</span><span>{breed.name}</span></nav>
      <div className={`breed-detail-card${breed.image ? "" : " breed-detail-card--placeholder"}`}>
        <div className="breed-detail-intro">
          ...
          <div className="breed-fci-path" aria-label="FCI zaradenie plemena">
            <Link href={fciGroupHref} className="breed-fci-path-node" aria-label={`FCI skupina ${breed.fciGroup}: ${fciGroupName}`}><small>FCI skupina {breed.fciGroup}</small><strong>{fciGroupName}</strong></Link>
            {(fciSectionNumber || fciSectionName) && <><span className="breed-fci-path-arrow" aria-hidden="true">→</span>{fciSectionHref ? <Link href={fciSectionHref} className="breed-fci-path-node" aria-label={`FCI sekcia ${fciSectionNumber}${fciSectionName ? `: ${fciSectionName}` : ""}`}><small>Sekcia {fciSectionNumber}</small>{fciSectionName && <strong>{fciSectionName}</strong>}</Link> : <span className="breed-fci-path-node"><small>FCI sekcia</small>{fciSectionName && <strong>{fciSectionName}</strong>}</span>}</>}
            <span className="breed-fci-path-arrow" aria-hidden="true">→</span><span className="breed-fci-path-current"><small>Plemeno</small><strong>{breed.name}</strong></span>
          </div>
        ...
        <dl className="breed-hero-facts">{breed.origin && <div><dt>Pôvod</dt><dd>{breed.origin}</dd></div>}{heroHeight && <div><dt>Výška</dt><dd>{heroHeight}</dd></div>}{heroWeight && <div><dt>Hmotnosť</dt><dd>{heroWeight}</dd></div>}{breed.lifespan && <div><dt>Dĺžka života</dt><dd>{breed.lifespan}</dd></div>}{fci.vyuzitie && <div><dt>Využitie</dt><dd>{fci.vyuzitie}</dd></div>}</dl>
      </div></header>
    ...
    {hasPracticalContent && <div className="breed-detail-body breed-detail-body--single shell"><div className="breed-copy">
      {sections.map((section) => <section id={section.id} className="breed-practical-section" key={section.id}><span className="eyebrow">{section.eyebrow}</span><div className="breed-section-heading"><span aria-hidden="true"><PawMark size={18}/></span><h2>{section.title}</h2></div>{paragraphs(section.content).map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}
