...
export default async function BreedDetailPage({ params }: Props) {
  ...
  return <main id="obsah">
    ...
    <section className="breed-useful-links shell"><h2>Užitočné odkazy a kontakty</h2><div><Link href="/adresar/chovatelske-kluby"><strong>Chovateľské kluby</strong><small>Kluby združujúce chovateľov a priaznivcov plemena.</small><span>Zobraziť kluby →</span></Link><Link href="/adresar/chovatelske-stanice"><strong>Chovateľské stanice</strong><small>Publikované stanice v databáze Psipedie.</small><span>Zobraziť stanice →</span></Link><Link href={`/adresar/treneri?breed=${encodeURIComponent(breed.name)}`}><strong>Psí tréneri</strong><small>Tréneri so skúsenosťami s pracovnými aj rodinnými psami.</small><span>Zobraziť trénerov →</span></Link><Link href="/adresar/kynologicke-kluby"><strong>Kynologické kluby</strong><small>Kluby pre šport, výcvik a praktické aktivity.</small><span>Zobraziť kluby →</span></Link></div></section>
