...
export default async function BreedDetailPage({ params }: Props) {
  ...
  const sections = [{id:"historia",eyebrow:"História",title:"História plemena",content:breed.history},
  ...
  return <main id="obsah">
    ...
    <header className="breed-detail-hero shell" id="prehlad"><nav className="article-breadcrumbs" aria-label="Navigácia"><Link href="/">Domov</Link><span>/</span><Link href="/plemena">Plemená</Link><span>/</span><span>{breed.name}</span></nav>
      <div className={`breed-detail-card${breed.image ? "" : " breed-detail-card--placeholder"}`}>
        ...
        {managedBreed?.editorialComplete && <div className="breed-hero-ratings" aria-label="Redakčné hodnotenie plemena"><RatingDots value={managedBreed.energy} label="Energia"/><RatingDots value={managedBreed.trainability} label="Cvičiteľnosť"/><RatingDots value={managedBreed.children??managedBreed.family} label="Rodina"/></div>}
    ...
    {hasPracticalContent && <div className="breed-detail-body breed-detail-body--single shell"><div className="breed-copy">
      {sections.slice(0,2).map((section) => <section id={section.id} className="breed-practical-section" key={section.id}><span className="eyebrow">{section.eyebrow}</span><div className="breed-section-heading"><span aria-hidden="true"><PawMark size={18}/></span><h2>{section.title}</h2></div>{paragraphs(section.content).map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}
      {(breed.goodFor.length>0||breed.consider.length>0)&&<section className="breed-practical-section breed-practical-fit" aria-label="Praktická vhodnosť plemena"><div className="breed-section-heading"><span aria-hidden="true"><PawMark size={18}/></span><h2>Pre koho je vhodný a pre koho nie</h2></div>{breed.goodFor.length>0&&<div className="is-positive"><h3>Hodí sa pre</h3><ul>{breed.goodFor.map((item)=><li key={item}>{item}</li>)}</ul></div>}{breed.consider.length>0&&<div className="is-caution"><h3>Treba zvážiť</h3><ul>{breed.consider.map((item)=><li key={item}>{item}</li>)}</ul></div>}</section>}
      {sections.slice(2).map((section) => <section id={section.id} className="breed-practical-section" key={section.id}><span className="eyebrow">{section.eyebrow}</span><div className="breed-section-heading"><span aria-hidden="true"><PawMark size={18}/></span><h2>{section.title}</h2></div>{paragraphs(section.content).map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}
