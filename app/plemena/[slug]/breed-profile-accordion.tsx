...
export function BreedProfileAccordion({ title, eyebrow, children, defaultOpen = false }: Props) {
  const reactId = useId();
  const id = `breed-profile-${reactId.replace(/:/g, "")}`;
  const [open, setOpen] = useState(defaultOpen);

  return (
    <section
      className={`${styles.accordion}${open ? ` ${styles.accordionOpen}` : ""}`}
      data-breed-accordion={title}
    >
