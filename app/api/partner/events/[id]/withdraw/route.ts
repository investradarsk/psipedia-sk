...
import { PartnerEventError, withdrawPartnerEventSubmission } from "@/lib/partner-events";
...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    const submission=await withdrawPartnerEventSubmission({accountId:identity.accountId,id});
    return Response.json({success:true,submission},{headers:{"Cache-Control":"private, no-store"}});
  }catch(error){
    ...
    return Response.json({error:status>=500?"Návrh momentálne nie je možné zrušiť.":error instanceof Error?error.message:"Požiadavka zlyhala."},{status,headers:{"Cache-Control":"private, no-store"}});
  }
}