...
export async function POST(request:Request,{params}:{params:Promise<{resourceId:string}>}){
  try{
    ...
    const {resourceId}=await params;if(!/^[A-Za-z0-9_-]{1,128}$/.test(resourceId))throw new PartnerEventError("Neplatný Partner resource.");
    const body=await request.json() as Record<string,unknown>;
    const submission=await submitPartnerEventUpdate({accountId:identity.accountId,resourceId,baseRevision:body.baseRevision,patch:body.patch});
