...
export function FciStandardAccordions({ groups }: Props) {
  ...
  return (
    <div className={styles.standardLayout}>
      <aside className={styles.standardSidebar} aria-label="Obsah FCI štandardu">
        <p>Obsah štandardu</p>
        <nav>
      ...
      <div className={styles.standardMain}>
        <div className={styles.standardControls} aria-label="Ovládanie FCI štandardu">
          <button type="button" onClick={() => setOpenIds(new Set(allIds))}>Rozbaliť celý štandard</button>
          <span aria-hidden="true">·</span>
          <button type="button" onClick={() => setOpenIds(new Set())}>Zbaliť všetko</button>
        </div>
        ...
        <div className={styles.standardAccordionList}>
          {groups.map((group, index) => {
            ...
            return (
              <section key={group.id} id={group.id} className={`${styles.standardAccordion}${open ? ` ${styles.standardAccordionOpen}` : ""}`}>
                <h2>
                  <button
                    type="button"
                    id={triggerId}
                ...
                >
                  <div className={styles.standardAccordionBody}>
                    {group.items.map((item, itemIndex) => (
                      <section className={styles.standardSubsection} key={`${group.id}-${item.label ?? itemIndex}`}>
                        ...
                        {item.paragraphs?.length ? (
                          <div className={styles.standardProse}>{item.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>
                        ) : null}
                      </section>
                    ))}
                  </div>
