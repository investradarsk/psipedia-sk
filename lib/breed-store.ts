...
export type ManagedBreedIndexItem = Pick<ManagedBreed, "id" | "slug" | "name" | "status" | "image" | "fciGroup" | "fciSection" | "origin" | "group" | "accent" | "intro" | "energy" | "trainability" | "family" | "officialFciName" | "searchText" | "editorialComplete" | "seo" | "updatedAt">;
...
type IndexRow = SummaryRow & { fci_section:string;intro:string;energy:number;trainability:number;family:number;search_text:string;editorial_complete:number;seo_json:string;updated_at:string };
...
function fromIndexRow(row:IndexRow):ManagedBreedIndexItem { return {
  ...fromSummaryRow(row),fciSection:row.fci_section,intro:row.intro,energy:row.energy,trainability:row.trainability,family:row.family,
