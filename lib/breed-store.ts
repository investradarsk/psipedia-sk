...
function fromRow(row:Row,relations:{articles?:number[];directory?:number[];public?:boolean}={}):ManagedBreed { const editorial=parseObject<BreedEditorial>(row.editorial_json);editorial.heroTraits=Array.isArray(editorial.heroTraits)?editorial.heroTraits.filter(isBreedHeroTrait).slice(0,3):[];const fciStandard=parseObject<FciStandard>(row.fci_standard_json);return {
  ...
  size:row.size,weight:relations.public?publicBreedMeasurement(row.weight,"weight",combinedFciMeasurement([fciStandard.hmotnost_pes_kg,fciStandard.hmotnost_suka_kg],"kg")):row.weight,height:relations.public?publicBreedMeasurement(row.height,"height",combinedFciMeasurement([fciStandard.vyska_pes_cm,fciStandard.vyska_suka_cm],"cm")):row.height,lifespan:relations.public?publicBreedMeasurement(row.lifespan,"lifespan"):row.lifespan,coat:row.coat,energy:row.energy,trainability:row.trainability,
...
export async function getBreedOfTheDay(dayOfYear:number):Promise<BreedOfTheDayItem|null>{
  try {
    const database=db();
    if(!database){const candidates=rotateBreeds(seedBreeds,dayOfYear);return candidates.find(breed=>breed.image.trim())??candidates[0]??null;}
    ...
    return row?{slug:row.slug,name:row.name,image:row.image,fciGroup:row.fci_group,fciSection:publicFciSectionName(row.fci_group,row.fci_section_number,row.fci_section),size:row.size,energy:row.energy,trainability:row.trainability,intro:row.intro}:null;
