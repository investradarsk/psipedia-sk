...
export type AdoptionD1Database = {
  prepare(query: string): AdoptionD1Statement;
};
...
type AdoptionDogRow = {
  ...
  breed_profile_name: string | null;
  breed_mix: number;
  color: string;
  region: string;
  district: string;
  ...
  suitable_for_dogs: string;
  suitable_for_cats: string;
  suitable_for_other_animals: string;
  apartment_suitable: number | null;
  beginner_suitable: number | null;
  needs_experienced_owner: number;
...
export type AdoptionPagination = {
  page: number;
  pageSize: number;
  total: number;
  totalPages: number;
...
function parseGallery(value: string) {
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter((item): item is string => typeof item === "string" && item.trim().length > 0).slice(0, 20)
      : [];
  } catch {
    return [];
  }
...
function rowToDog(row: AdoptionDogRow): AdoptionDog {
  ...
  return {
    ...
    name: row.name,
    slug: row.slug,
    status: normalizeRowChoice(row.status, adoptionStatuses, "DRAFT"),
    sex: normalizeRowChoice(row.sex, adoptionSexes, "UNKNOWN"),
    birthDate: row.birth_date,
    approximateAgeMonths: row.approximate_age_months,
    size: normalizeRowChoice(row.size, adoptionSizes, "UNKNOWN"),
    weight: row.weight,
    breedId: row.breed_id,
    breedName: normalizeAdoptionText(row.breed_profile_name) || row.breed_name,
    breedSlug: normalizeAdoptionText(row.breed_slug) || null,
...
function dogToInput(dog: AdoptionDog): ManagedAdoptionInput {
  return {
    ...
    approximateAgeMonths: dog.approximateAgeMonths,
    size: dog.size,
    weight: dog.weight,
    breedId: dog.breedId,
    breedName: dog.breedName,
    breedMix: dog.breedMix,
    color: dog.color,
    region: dog.region,
    ...
    organizationSlug: dog.organizationSlug,
    ...
    suitableForChildren: dog.suitableForChildren,
    suitableForDogs: dog.suitableForDogs,
    suitableForCats: dog.suitableForCats,
    suitableForOtherAnimals: dog.suitableForOtherAnimals,
    apartmentSuitable: dog.apartmentSuitable,
    beginnerSuitable: dog.beginnerSuitable,
    needsExperiencedOwner: dog.needsExperiencedOwner,
    ...
    externalSourceUrl: dog.externalSourceUrl,
    contactEmail: dog.contactEmail,
    contactPhone: dog.contactPhone,
    contactUrl: dog.contactUrl,
    lastVerifiedAt: dog.lastVerifiedAt,
  };
}
...
export async function prepareAdoptionWritePayload(
  ...
  return {
    ...preparedBase,
    breedSlug: breed.breedSlug,
    searchText,
    publishedAt,
    updatedAt: timestamp,
    updatedBy: normalizeAdoptionText(editorEmail).toLowerCase(),
  };
}

export function buildPublicAdoptionQuery(filters: AdoptionPublicFilters = {}) {
  const page = Math.max(1, Math.trunc(Number(filters.page) || 1));
  const conditions = ["d.status IN ('ACTIVE','RESERVED')"];
  const bindings: unknown[] = [];
  const q = normalizeAdoptionSearchText([filters.q]);
  if (q) {
    conditions.push("d.search_text LIKE ?");
    bindings.push(`%${q}%`);
  }
  const region = normalizeAdoptionText(filters.region);
  if ((adoptionRegions as readonly string[]).includes(region)) {
    conditions.push("d.region = ?");
    bindings.push(region);
  }
  if (filters.sex && isAdoptionSex(filters.sex) && filters.sex !== "UNKNOWN") {
    conditions.push("d.sex = ?");
    bindings.push(filters.sex);
  }
  if (filters.size && isAdoptionSize(filters.size) && filters.size !== "UNKNOWN") {
    conditions.push("d.size = ?");
    bindings.push(filters.size);
  }
  ...
  return {
    ...
    orderBy,
    page,
    pageSize: ADOPTION_PAGE_SIZE,
    offset: (page - 1) * ADOPTION_PAGE_SIZE,
  };
}
...
export async function getPublicAdoptionBySlug(slug: string, database?: AdoptionD1Database) {
  ...
  if (!db) return null;
  const row = await db
    .prepare(`${PUBLIC_SELECT} WHERE d.slug = ? AND d.status IN ('ACTIVE','RESERVED') LIMIT 1`)
    .bind(normalized)
    .first<AdoptionDogRow>();
  return row ? rowToDog(row) : null;
}

export async function listPublicAdoptions(filters: AdoptionPublicFilters = {}, database?: AdoptionD1Database) {
  ...
  const countRow = await db
    .prepare(`SELECT COUNT(*) AS count FROM adoption_dogs d WHERE ${query.where}`)
    .bind(...query.bindings)
    .first<{ count: number }>();
  const total = Number(countRow?.count ?? 0);
  const result = await db
    .prepare(`${PUBLIC_SELECT} WHERE ${query.where} ORDER BY ${query.orderBy} LIMIT ? OFFSET ?`)
    .bind(...query.bindings, query.pageSize, query.offset)
    .all<AdoptionDogRow>();
  return {
    items: result.results.map(rowToDog),
...
export async function listManagedAdoptions(filters: AdoptionAdminFilters = {}, database?: AdoptionD1Database, now = new Date()) {
  ...
  const total = Number(countRow?.count ?? 0);
  const result = await db
    .prepare(`SELECT id, name, slug, status, sex, size, city, region, organization_name, main_image, last_verified_at, updated_at
      FROM adoption_dogs ${query.where} ORDER BY updated_at DESC, id DESC LIMIT ? OFFSET ?`)
    .bind(...query.bindings, query.pageSize, query.offset)
    .all<AdoptionSummaryRow>();
  const threshold = new Date(now.getTime() - ADOPTION_STALE_DAYS * 86_400_000).toISOString();
  const staleRow = await db
    .prepare("SELECT COUNT(*) AS count FROM adoption_dogs WHERE status IN ('ACTIVE','RESERVED') AND (last_verified_at IS NULL OR last_verified_at < ?)")
    .bind(threshold)
    .first<{ count: number }>();
  ...
  const items: ManagedAdoptionSummary[] = result.results.map((row) => ({
    ...
    size: normalizeRowChoice(row.size, adoptionSizes, "UNKNOWN"),
    city: row.city,
    region: (adoptionRegions as readonly string[]).includes(row.region) ? row.region as AdoptionRegion : "",
    organizationName: row.organization_name,
    mainImage: row.main_image,
    lastVerifiedAt: row.last_verified_at,
    updatedAt: row.updated_at,
  ...
  return {
    items,
    pagination: { page: query.page, pageSize: query.pageSize, total, totalPages: Math.ceil(total / query.pageSize) } satisfies AdoptionPagination,
    counts,
    staleCount: Number(staleRow?.count ?? 0),
  };
}

async function writeAdoption(
  database: AdoptionD1Database,
  payload: ManagedAdoptionInput,
  editorEmail: string,
  existing: AdoptionDog | null,
) {
  ...
  if (!existing) {
    ...
    const row = await database
      .prepare(`INSERT INTO adoption_dogs (${columns.join(", ")}) VALUES (${columns.map(() => "?").join(", ")}) RETURNING id`)
      .bind(...values, prepared.updatedAt, prepared.updatedBy)
      .first<{ id: number }>();
    if (!row) throw new Error("Profil psa sa nepodarilo vytvoriť.");
    return getAdoptionById(Number(row.id), database);
  }
  await database
    .prepare(`UPDATE adoption_dogs SET ${MUTABLE_COLUMNS.map((column) => `${column} = ?`).join(", ")} WHERE id = ?`)
...
export async function transitionManagedAdoptionStatus(
  id: number,
