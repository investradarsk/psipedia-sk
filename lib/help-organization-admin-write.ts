...
export function buildOrganizationCreateStatement(
  ...
  const values = [
    input.name, input.slug, input.legalName, input.registrationNumber, input.type, "DRAFT",
  ...
  if (!guard) {
    return database.prepare(`INSERT INTO help_organizations (${columns}) VALUES (${values.map(() => "?").join(",")})`).bind(...values);
  }
