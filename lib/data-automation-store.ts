...
export async function finishAutomationRun(input: {
  ...
  await db.batch([
    ...
    db.prepare(`UPDATE automation_sources SET last_checked_at=?,last_success_at=?,
      last_error_at=?,last_error_code=?,next_check_at=?,updated_at=? WHERE id=?`).bind(
      completedIso,
      input.status === "SUCCESS" ? completedIso : null,
      input.status === "FAILED" ? completedIso : null,
      input.status === "FAILED" ? sourceErrorCode : null,
      nextCheckAt,
      completedIso,
      input.source.id,
    ),
