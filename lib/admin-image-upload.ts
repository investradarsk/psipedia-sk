...
async function optimizeImage(file: File) {
  ...
  if (file.size > MAX_ADMIN_IMAGE_BYTES) throw new Error(`Obrázok má ${formatMegabytes(file.size)}. Maximum je 8 MB.`);
...
export async function uploadAdminImage(file: File, folder: AdminImageFolder): Promise<AdminImageUpload> {
  ...
  const response = await fetch("/api/admin/uploads", {
    method: "POST",
    headers: { "content-type": fileType(uploadFile), "x-upload-folder": folder, "x-file-name": encodeURIComponent(uploadFile.name.slice(0, 160)), "x-original-size": String(originalBytes) },
    body: uploadFile,
  });
  let data: Partial<AdminImageUpload> & { error?: string } = {};
  try { data = await response.json() as Partial<AdminImageUpload> & { error?: string }; } catch {}
  ...
  return { imageUrl: data.imageUrl, imageKey: data.imageKey, filename: data.filename || uploadFile.name, originalBytes, storedBytes: data.storedBytes || uploadFile.size, optimized: uploadFile.size < originalBytes };
}

export function adminImageUploadMessage(result: AdminImageUpload, nextStep: string) {
  const optimization = result.optimized ? ` Optimalizovaný z ${formatMegabytes(result.originalBytes)} na ${formatMegabytes(result.storedBytes)}.` : "";
