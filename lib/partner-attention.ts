...
export async function loadPartnerPendingSummary(database?:D1Database){
  try{
    ...
    const [claimRow,profileChangeRow,verificationRow,commercialRow]=await Promise.all([
      ...
      db.prepare("SELECT COUNT(*) count FROM moderation_submissions s JOIN partner_profile_change_metadata m ON m.submission_id=s.id WHERE s.resource_type IN ('DIRECTORY_PROFILE','HELP_ORGANIZATION') AND s.operation='UPDATE' AND s.submitter_type='PARTNER_ACCOUNT' AND s.status IN ('SUBMITTED','PENDING_REVIEW','QUARANTINED')").first<{count:number}>(),
